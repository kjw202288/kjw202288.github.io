...
0. ECE로 성형을 한뒤 반드시 슬롯에 저장한다 그렇지 않으면 npc파일을 추출할때 성형데이터가 저장이 되지 않는다 또한, 캐릭터의 키의 값을 기억해둔다
...
Essential과 Unique를 선택. Race는 NordRace(Race 임의), Skin은 0_SkinNaked(Custom). Female을 선택(성별 임의), 키의 값을 지정해줌.

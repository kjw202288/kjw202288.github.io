...
3. 스카이림의 경우 아머의 메시는 ~_1.nif를 추출하고 여성 캐릭터의 경우 ~f_1.nif를 추출한다 아머의 텍스쳐는 .dds, _m.dds, _n.dds 등을 추출한다
...
7. Assets 부분에서 오른쪽 마우스 클릭하여 Create - Rendering - Material을 선택하여 Material을 만들어준다 Shader는 Standard(Specular setup)를 선택해준다 스카이림의 경우 Albedo 부분에 .dds를 드래그하고  노멀맵과 스페큘러맵 적용이 필요한 경우 _n.dds와 _msn.dds는 Normal map 부분에 드래그하고 _s는 Specualr 부분에 드래그하여준다 Smoothness는 0으로 조정한다 또한 틴트 텍스쳐나 스킨톤 텍스쳐가 따로있는 경우 헤드나 바디 텍스쳐에 투명도를 조절해서 합쳐준다 드래그가 불가능한 텍스쳐들이 있다면 paint.net 같은 프로그램으로 dds 확장자에서 tga 확장자로 바꿔준다. 텍스쳐를 불러왔는데 위아래로 뒤집혀있다면 쉐이더에 적용할때 Tiling에서 Y부분을 -1로 바꿔준다 불투명한 매테리얼은 Rendering Mode를 Opaque로 그냥 두고 투명, 반투명이 필요한 텍스쳐가 들어가는 매테리얼은 Rendering Mode를 Transparent나 Cutout으로 바꿔준다 또한 nif파일마다 텍스쳐가 다르므로 NifSkope를 활용하여 뒤에서의 파츠별 매테리얼 적용을 고려하여 알맞은 텍스쳐를 찾아 드래그해준다

...
1. Applio Github에서 코랩 webui 버전의 Applio.ipynb를 다운받거나 로컬 webui 버전을 다운받는다 <https://github.com/IAHispano/Applio?tab=readme-ov-file> 로컬 버전의 경우 미니콘다를 설치한다. 윈도우 사용자 이름이 한글명인 경우 영어 경로로 설치하고 시작 버튼-시스템 환경 변수 편집-환경 변수-Path 편집(사용자명에 대한 환경변수)으로 들어가서
```
C:\Miniconda\
...
3. 모델을 학습시키거나 다운로드하여 원하는 모델을 적용시킬수 있다
...
4. TTS에 들어가 원하는 음성모델을 선택하고 TTS음성에서 성별을 고른뒤 합성할 텍스트를 적어 변환하여 커스텀보이스 결과물을 얻는다
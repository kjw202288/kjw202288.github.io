...
- Data/meshes/actors/character/FaceGenData/FaceGeom/(Custom).esp에 경로의 .nif파일을 열어 nifskope 상에서 Head,Eye,Brows,Hair,Hairline,mouth의 NiNode - ~Shape - BSLightingShader~ - BSShaderTexture 부분을 선택. Block List의 Block Details 부분의 Textures의 경로 부분을 일전에 복붙한 경로로 재지정한다 이 과정은 일전에 복붙한 Hair의 Meshes 파일인 nif파일에서도 해줘야한다 Data\textures\Actors\Character\FaceGenData\FaceTint\(Custom).esp 부분에서 틴트파일의 경로도 재지정해준다
